---
name: release-pocketkuma
description: Prepare, publish, and verify PocketKuma releases through the repository's Bun build and GitHub Actions workflow. Use when creating the next stable, beta, or release-candidate version; bumping the application version; pushing a version tag; or checking generated GitHub release notes, binaries, and checksums.
---
...
- Expect six binaries plus `checksums-sha256.txt`:
    ...
    - `pocketkuma-linux-arm64`
    - `pocketkuma-linux-x64-musl`
    - `pocketkuma-darwin-x64`
    - `pocketkuma-darwin-arm64`
    - `pocketkuma-windows-x64.exe`
...
```bash
git tag -a vVERSION COMMIT -m "PocketKuma vVERSION"
