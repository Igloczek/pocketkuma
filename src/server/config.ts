...
const hostname = args.host || process.env.POCKETKUMA_HOST || hostEnv;

const port = [args.port, process.env.POCKETKUMA_PORT, process.env.PORT, 3001]
    .map((portValue) => parseInt(portValue))
    .find((portValue) => !isNaN(portValue));

const sslKey = args["ssl-key"] || process.env.POCKETKUMA_SSL_KEY || process.env.SSL_KEY || undefined;
const sslCert = args["ssl-cert"] || process.env.POCKETKUMA_SSL_CERT || process.env.SSL_CERT || undefined;
const sslKeyPassphrase =
    args["ssl-key-passphrase"] ||
    process.env.POCKETKUMA_SSL_KEY_PASSPHRASE ||
